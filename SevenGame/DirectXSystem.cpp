...
///////////////////////////////////////////////////////////////////////////////////
	...
	m_SevenGameManger = new CSGManager();
	m_SevenGameManger->AllocateObjcet(4,3); // create four User;
...
BOOL CDirectXSystem::ProcessMessage( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		...
		m_iMouseY       = HIWORD(lParam) < GetHeight() ? HIWORD(lParam) : 0;
...
//End
	...
	CSGUser* pHumanUser = NULL;

//Draw User Card
	CDataList<CSGUser *>::POSITION pEnd = m_SevenGameManger->m_userlist.GetEndPosition();	
	CDataList<CSGUser *>::POSITION pos  = m_SevenGameManger->m_userlist.GetHeadPosition();
	while ( pos != pEnd) 			
	{
		CSGUser* pUser = (CSGUser*)m_SevenGameManger->m_userlist.GetType(pos);		
		if( NULL != pUser)
		{
			int CardNum = pUser->GetRemainCard();				
			int iPassTicket = pUser->GetPassTicketNum();

			if( pUser->GetID() == SILENCE_PLAYER )	
			{
			...
			if( pUser->GetID() == DETECTIVE_PLAYER )	
			{
				for(int i=0;i<CardNum;i++)			
		...
		pos = m_SevenGameManger->m_userlist.GetNextPosition(pos);				
	} 
...
//Draw User Card
	...
	if(pHumanUser)
	{						
		iCardNum = pHumanUser->GetRemainCard();
		iPassticket = pHumanUser->GetPassTicketNum();
		m_SevenGameManger->ReArrangeUserCard(pHumanUser,iCount,iCardNum);
	}
	...
	if( iPassticket >= 0 )
	{
		...
		{	
			SPRITE_CARD_INFO* pCardInfo = m_SevenGameManger->GetUserCard();


			pCardSet->OnRenderRegion(fElapsedTime, 
				                          HUMAN_PLAYER_POS_X + (i-((iCardNum)/2))*CARD_LOOKABLE_PART, 
										  HUMAN_PLAYER_POS_Y,
...
{
	CSGUser *pUser = m_SevenGameManger->GetCurrentUser();
	...
	else pAITurn->OnRender(fElapsedTime);
...
BOOL CDirectXSystem::DrawWinLose( float fElapsedTime )
{
	...
	if(m_SevenGameManger->m_pUserRanking[HUMAN_PLAYER] == 1)
	{
